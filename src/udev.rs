...
type UdevRenderer<'a, 'b> = MultiRenderer<
    'a,
    'a,
    'b,
    GbmGlesBackend<Gles2Renderer>,
    GbmGlesBackend<Gles2Renderer>,
    Gles2Renderbuffer,
>;
...
pub fn run_udev() {
    ...
    let udev_backend = match UdevBackend::new(&state.seat_name) {
        Ok(ret) => ret,
        Err(err) => {
            tracing::error!(err, "Failed to initialize udev backend");
            return;
        }
    };
        ...
        #[cfg(feature = "egl")]
        {
            ...
            if renderer.bind_wl_display(&display.handle()).is_ok() {
                ...
                let global = dmabuf_state
                    .create_global::<AnvilState<UdevData>, _>(&display.handle(), dmabuf_formats);
...
impl AnvilState<UdevData> {
    ...
    fn render(&mut self, dev_id: DrmNode, crtc: Option<crtc::Handle>) {
        let device_backend = match self.backend_data.backends.get_mut(&dev_id) {
            Some(backend) => backend,
            None => {
                tracing::error!("Trying to render on non-existent backend {}", dev_id);
                return;
            }
        };
